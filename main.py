...
def find_sku(message):
    bot.send_message(
        message.chat.id,
        "Ваш менеджер: Фирсов Иван\nЭлектронная почта: info@texsar.net\nТелефон:8 800 600 74 66",
    )


# Поиск по 1С: УНФ и вывод названия, количества и сегодняшней даты
@bot.message_handler()
def check_sku(message):
    ...
    if findsku == 1:
        ...
        sku_url = (
            one_c_url
            + "Catalog_Номенклатура?$filter=Артикул eq '"
            + sku
            + "'&$select=Ref_Key, Артикул, Description&$format=json"
        )
        ...
        if json_data["value"] == []:
            bot.send_message(
                message.chat.id,
                f"Товара с артикулом {sku} нет наличии. О возможности поставки уточните, пожалуйста, у менеджера по почте info@texsar.ru, либо по телефону 8 800 600 74 66",
            )
        else:
            # now = datetime.now()
            ...
            quantity_url = (
                one_c_url
                + "InformationRegister_ОстаткиТоваров?$filter=Номенклатура_Key eq guid'"
                + Ref_Key
                + "'&$format=json&$select=Количество, СтруктурнаяЕдиница"
            )
            ...
            if json_data2["value"] == []:
                ...
                bot.send_message(
                    message.chat.id, f"{name} в наличии в количестве {quantity} шт.."
                )
        findsku = 0
    else:
